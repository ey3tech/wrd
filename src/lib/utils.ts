...
const punctuation = `!"#$%&'()*+,-./:;<=>?@[\\]^_\`{|}~`;
const complex_punctuation = punctuation.split('').filter(char => !basic_punctuation.includes(char)).join('');
...
function calculatePoolSize(password: string) {
    ...
    if (chars.some(char => ascii_uppercase.includes(char))) pool_size += ascii_uppercase.length;
    if (chars.some(char => ascii_lowercase.includes(char))) pool_size += ascii_lowercase.length;
    if (chars.some(char => basic_punctuation.includes(char))) pool_size += basic_punctuation.length;
    if (chars.some(char => complex_punctuation.includes(char))) pool_size += complex_punctuation.length;
    return pool_size;
}
export function calculateEntropy(password: string) {
    if (password.length === 0) return 0; // entropy is 0 if there's no password

    let pool_size = calculatePoolSize(password);
...
export function calculateStrength(password: string) {
    ...
    const complexityFactor = Math.min((new Set(password.split('')).size / ((poolSize || 1)))*2.5, 1); // consider passwords with more character variety stronger
    
    return Math.min(((entropy*lengthFactor*complexityFactor) / ideal_entropy), 1);
