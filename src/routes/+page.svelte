<script lang="ts">
    ...
    let passwordStrength = writable(0);
    let passwordEntropy = writable(0);
    let crackTime = writable("instant");
    ...
    $: if (password.length > 50) {
        ...
        if (password.length === 0) {
            ...
            message.set(randomChoice($passwordStrength > 60 ? positiveText: negativeText))
        }
    }
</script>

<section class="fullscreen relative bg-cover text-white " style="background-image: url('bg.svg')">
    <h1 class="text-6xl my-0 font-sans">oh wrd?</h1>
    {#key $message}
    <p class="text-lg font-sans w-2/3 text-center" in:fade={{duration: 300}}>
        {$message}
    </p>
    {/key}
